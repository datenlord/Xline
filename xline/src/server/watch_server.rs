...
use crate::{
    rpc::{RequestUnion, ResponseHeader, Watch, WatchRequest, WatchResponse},
...
impl WatchHandle {
    ...
    /// Get next available `WatchId`, return -1 if id is not available
    fn get_next_watch_id(&mut self, id: WatchId) -> WatchId {
            ...
            match req {
                RequestUnion::CreateRequest(req) => {
                    ...
                    let (watcher, events, revision) = self
                        ...
                        .await;
                    let _prev = self.watcher_map.insert(watch_id, watcher);
                ...
                RequestUnion::CancelRequest(req) => {
                    ...
                    if let Some(watcher) = self.watcher_map.get(&watch_id).cloned() {
                        let revision = self.kv_watcher.cancel(watcher).await;
