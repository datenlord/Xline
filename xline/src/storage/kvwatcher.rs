...
use crate::{rpc::Event, server::command::KeyRange, storage::kvstore::KvStoreInner};
...
impl Watcher {
    ...
    /// Filter event
    pub(crate) fn filter_events(&self, events: Vec<Event>) -> Vec<Event> {
        events
            .into_iter()
            .filter(|event| {
                !self
...
impl KvWatcher {
    ...
    pub(crate) async fn cancel(&self, watcher: Watcher) -> i64 {
        self.inner.cancel(watcher).await
    }
}

impl KvWatcherInner {
    /// New `KvWatchInner`
    fn new(storage: Arc<KvStoreInner>) -> Self {
        Self {
            storage,
    ...
    /// Cancel a watch from KV store
    async fn cancel(&self, watcher: Watcher) -> i64 {
        let revision = self.storage.revision();
    ...
    async fn handle_kv_updates(&self, updates: (i64, Vec<Event>)) {
        ...
        let watcher_map = self.watcher_map.lock().await;
        for (k, v) in watcher_map.iter() {
            ...
            if !events.is_empty() {
                for watcher in v.iter() {
                    if revision < watcher.start_rev() {
                        continue;
                    }
                    let filtered_event = watcher.filter_events(events.clone());
